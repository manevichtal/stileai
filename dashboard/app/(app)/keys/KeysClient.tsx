    ...
    <div className="p-7 flex flex-col gap-6 max-w-[820px]">
      <div className="bg-bg2 border border-line rounded-[14px] p-4 flex items-end gap-3">
        <label className="flex-1 flex flex-col gap-1">
          <span className="font-mono text-[11px] text-ink2 font-medium">Create a key</span>
      ...
      {freshKey && (
        <div className="border border-blue/40 bg-bluedim rounded-[14px] p-4">
          <div className="font-mono text-[11.5px] text-ink2 mb-2">
          ...
          <div className="flex items-center gap-2">
            <code className="flex-1 font-mono text-[12px] text-ink bg-card border border-line rounded-lg px-3 py-2 break-all">{freshKey}</code>
            <button
              onClick={() => { navigator.clipboard.writeText(freshKey); setCopied(true); setTimeout(() => setCopied(false), 1500); }}
              className="font-mono text-[11.5px] text-blue border border-blue/40 rounded-lg px-3 py-2 hover:bg-card"
            >
              {copied ? "Copied" : "Copy"}
            </button>
            <button onClick={() => setFreshKey(null)} className="font-mono text-[11.5px] text-ink3 px-2">dismiss</button>
          </div>
        </div>
      )}
      ...
      <div className="border border-line rounded-[14px] overflow-hidden bg-card">
        <table className="w-full border-collapse">
          ...
          <tbody>
            {keys.length === 0 && (
              <tr><td colSpan={5} className="font-mono text-[12.5px] text-ink3 text-center py-10">No keys yet. Generate one to connect your checkpoint.</td></tr>
            )}
            {keys.map((k) => (
              <tr key={k.id} className="border-b border-line last:border-0">
                <td className="px-3.5 py-3 font-mono text-[12px] text-ink">{k.label}</td>
                <td className="px-3.5 py-3 font-mono text-[12px] text-ink3">{k.key_prefix}…</td>
                <td className="px-3.5 py-3 font-mono text-[11px] text-ink3">{new Date(k.created_at).toLocaleDateString()}</td>
                <td className="px-3.5 py-3 font-mono text-[11px] text-ink3">{k.last_used_at ? new Date(k.last_used_at).toLocaleString() : "never"}</td>
...
function RevokeButton({ id, onDone }: { id: string; onDone: () => void }) {
  ...
  if (!confirming)
    return <button onClick={() => setConfirming(true)} className="font-mono text-[11.5px] text-ink3 hover:text-slate">Revoke</button>;
  return (
